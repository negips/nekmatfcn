...
!     Mattias; Uzawa step
c         call incomprp (vxp(1,jp),vyp(1,jp),vzp(1,jp),prp(1,jp))
...
c
      if (ifadj) then 
         do i=1,ntot1
            ta(i,1,1,1)=(vxp(i,jp)*g_adj(1)+vyp(i,jp)*g_adj(2)
     $           +vzp(i,jp)*g_adj(3))*beta_b !Pr g/|g|.u
         enddo 
      else
...
c
      ...
      if (ifadj) then
         call addcol3 (bqp(1,ifield-1,jp),bm1,ta,ntot1)
      else
