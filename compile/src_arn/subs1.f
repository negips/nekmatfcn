...
C
     ...
     $              , tw(lx1*ly1*lz1*lelv)
      ...
      if (.not.ifaxis) then
       if (ifsplit) then
!     Clio; initlial optimal condition with temperature
         call axhelm(au1,u1,h1,h2,IMESH,1)
         call axhelm(au2,u2,h1,h2,IMESH,2)
         if (if3d) call axhelm(au3,u3,h1,h2,IMESH,3)
c$$$         call axhelm(au1,u1,h1,h2,1,1)
c$$$         call axhelm(au2,u2,h1,h2,1,2)
c$$$         if (if3d) call axhelm(au3,u3,h1,h2,1,3)
         return
       endif
