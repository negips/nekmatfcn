...
!     read parameters fluid-structure interaction 
      subroutine psn_arnoldi_param_in(fid)
      implicit none
...
!     namelists
      namelist /PSNARN/ ifparnoldi,northo,psn_uzawa,parnoldi_ifpr,
     $                  ngs,pstep,pinistep 

!     default values
      ifparnoldi        = .FALSE.        ! if perform my Arnoldi
      psn_uzawa         = .TRUE.         ! uzawa at the first time step?
      parnoldi_ifpr     = .TRUE.         ! if include pressure in
                                         ! arnoldi vector
      northo            = lortho         ! no of vectors to save
...
!     read the file
      ierr=0
      if (NID.eq.0) then
         read(unit=fid,nml=PSNARN,iostat=ierr)
      endif
      call err_chk(ierr,'Error reading PSNARN parameters.$')

!     broadcast data
      call bcast(ifparnoldi    , LSIZE)
      call bcast(psn_uzawa     , LSIZE)
      call bcast(parnoldi_ifpr , LSIZE)
      ...
      return
      end subroutine psn_arnoldi_param_in
!----------------------------------------------------------------------
!     write parameters fluid-structure interaction
      subroutine psn_arnoldi_param_out(fid)
      implicit none
...
!     read the file
      ierr=0
      if (NID.eq.0) then
         write(unit=fid,nml=PSNARN,iostat=ierr)
      endif
      call err_chk(ierr,'Error writing PSNARN parameters.$')

      return
      end subroutine psn_arnoldi_param_out
c-----------------------------------------------------------------------

      subroutine psn_arnoldi_main
      ...
      real psn_innerprod           ! function
      real psn_wt_innerprod

!     Do nothing if its not on
      if (.not.ifparnoldi) return

      IFUZAWA = .false.
      if (northo.gt.lortho) then
        call exitti('northo > lortho, $', northo)
      endif
...
!     Remove orthogonal projections
      ...
      do igs=1,ngs            ! No of GS passes
        do j=1,nkryl
          hj = psn_wt_innerprod(Ax,Qortho(1,j),vlen) ! find projection
...
!     Add residual to hessenberg Matrix       
      beta = psn_wt_innerprod(Ax,Ax,vlen)
      beta = sqrt(beta)
      hessen(i+1,i)=beta
...
!     Reset time-stepping
      call psn_stepper_restart       

      if (nkryl.eq.northo+1) then
        call psn_arnoldi_finalize
      endif

      return
      end subroutine psn_arnoldi_main
!----------------------------------------------------------------------

      subroutine psn_arnoldi_init
...
!     Normalize starting vector
      vnorm = psn_wt_innerprod(Ax,Ax,vlen)
      vnorm = sqrt(vnorm)
...
!     Restart stepper
      ...
      return
      end subroutine psn_arnoldi_init
...
!---------------------------------------------------------------------- 
      ...
      real function psn_innerprod(x,y,n)
      ...
      psn_innerprod=glsc2(x,y,n)

      end function psn_innerprod
!---------------------------------------------------------------------- 

      real function psn_wt_innerprod(x,y,n)
      ...
      psn_wt_innerprod=glsc3(x,y,ArWt,n)


      end function psn_wt_innerprod
!---------------------------------------------------------------------- 
...
!----------------------------------------------------------------------
      ...
      do i=1,nkryl
        r(i)=psn_wt_innerprod(Qortho(1,i),Qortho(1,nkryl),vlen)
      enddo
...
!----------------------------------------------------------------------
      ...
      subroutine psn_arnoldi_finalize
     ...
     $          Qortho(1+2*ntot,i),prp,tp,'qor')
      ...
      return
      end subroutine psn_arnoldi_finalize
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
      subroutine psn_stepper_restart
...
!     set vxp,psi etc from Ax      
      ...
      return
      end subroutine psn_stepper_restart
!---------------------------------------------------------------------- 
