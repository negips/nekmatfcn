...
!     namelists
      namelist /CHKPOINT/ CHKPTSTEP,IFCHKPTRST,FULL_PR_IN,FULL_PR_OUT
!-----------------------------------------------------------------------
!     default values
      CHKPTSTEP = 100
      IFCHKPTRST = .FALSE.
...
!        Prabal
         if (full_pr_out) then
           ...
           if_full_pres = .false. ! map pressure to velocity mesh. prabal
         endif
...
c     perturbation from 'rs' files in all steps
         ...
         elseif (IFMHD) then    ! MHD
            ...
            call restart(1)
!           Below Added by prabal.
            if (full_pr_in) then
              if_full_pres = .true. ! map pressure to velocity mesh. prabal
            else
              if_full_pres = .false. ! map pressure to velocity mesh. prabal
            endif
