...
!        call spectrnsfm(f_filter,rt_kut,rt_wght)
      ...
      if (ifpert) then
!       Only add from jp=1
!       Must be called from inside makefp if this is needed for multiple
!       perturbation modes or if baseflow is also evolving                
        call opcopy(rtfx,rtfy,rtfz,vxp,vyp,vzp)
      else
