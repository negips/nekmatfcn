...
!    surf_lnvar = 3
!-------------------------------------------------- 
!     Surface Pressure^2   ! testing 2nd variable 
      surf_lnvar = surf_lnvar + 1
      surf_npos = surf_lnvar
...
!--------------------------------------------------
 ...
 9005 format(A24,2X,I5,2X,A7,2X,E14.8E2)
...
!---------------------------------------------------------------------- 
      ...
      integer i,j,ix,it
      integer isf,sel,selno,selno2,svar
      ...
      integer tstampofst                        ! current offset
...
!     File I/O
 ...
 9002      format (A9,I2.2,A1,E14.8E2,A4)
           ...
           open(unit=funits(i), FILE=fname,form="unformatted",
...
!                  write(6,*) 'recpos', recpos         ! prabal
                  do ix=1,LX1
                    recpos=hdrofst + tstampofst + 2*counter + ix
!                    write(6,*) recpos                  ! prabal
                    write(funits(isf),rec=recpos) SURF_XM1(ix,sel,isf)
                  enddo
                  do ix=1,LX1 
                    recpos=hdrofst + tstampofst + 2*counter + LX1 + ix
!                    write(6,*) recpos                  ! prabal
...
!     XM1
                   do ix=1,LX1
                     recpos=hdrofst + tstampofst + 2*cnt_rsum(isf)*LX1 
     $               + 2*(sel-1)*LX1 + ix
          ...
          do iz=kz1,kz2
            do iy=ky1,ky2
              do ix=kx1,kx2
                ...
                svn1(ix,iy,iz,iel) = sn*svar(ix,iy,iz,iel) 
