...
!         endif                  ! exclude unwanted elements
      ...
      iftimedep=.false.
      call user_stat_coords(iftimedep)

      return
      end subroutine user_stat_init

!----------------------------------------------------------------------

      subroutine user_stat_coords(iftimedep) 
...
!     local variables
      integer e                 ! loop indexes
      integer el                ! mapping index
      integer ifc               ! face number
      logical iftimedep         ! time-dependent?    
...
!     fill in 2D element mesh arrays
      if (STAT_IDIR.EQ.1) then  ! uniform X
        ...
        do e=1,NELV
          if(STAT_LMAP(e).ne.-1) then
            if (iftimedep) then
              el=STAT_LMAP(e)    
              call ftovec(STAT_XMT(1,1,el),YM1,e,ifc,NX1,NY1,NZ1)
              call ftovec(STAT_YMT(1,1,el),ZM1,e,ifc,NX1,NY1,NZ1)
            else
              el=e    
              call ftovec(STAT_XM1(1,1,el),YM1,e,ifc,NX1,NY1,NZ1)
              call ftovec(STAT_YM1(1,1,el),ZM1,e,ifc,NX1,NY1,NZ1)
            endif  
          endif
        enddo
      elseif (STAT_IDIR.EQ.2) then  ! uniform Y
        ...
        do e=1,NELV
          if(STAT_LMAP(e).ne.-1) then
            if (iftimedep) then
              el=STAT_LMAP(e)    
              call ftovec(STAT_XMT(1,1,el),XM1,e,ifc,NX1,NY1,NZ1)
              call ftovec(STAT_YMT(1,1,el),ZM1,e,ifc,NX1,NY1,NZ1)
            else
              el=e    
              call ftovec(STAT_XM1(1,1,el),XM1,e,ifc,NX1,NY1,NZ1)
              call ftovec(STAT_YM1(1,1,el),ZM1,e,ifc,NX1,NY1,NZ1)
            endif  
          endif
        enddo
        ...
        do e=1,NELV
          if(STAT_LMAP(e).ne.-1) then
            if (iftimedep) then
              el=STAT_LMAP(e)    
              call ftovec(STAT_XMT(1,1,el),XM1,e,ifc,NX1,NY1,NZ1)
              call ftovec(STAT_YMT(1,1,el),YM1,e,ifc,NX1,NY1,NZ1)
            else
              el=e    
              call ftovec(STAT_XM1(1,1,el),XM1,e,ifc,NX1,NY1,NZ1)
              call ftovec(STAT_YM1(1,1,el),YM1,e,ifc,NX1,NY1,NZ1)
            endif  
          endif
        enddo
      endif
      ...
      return
      end subroutine user_stat_coords
c----------------------------------------------------------------------
