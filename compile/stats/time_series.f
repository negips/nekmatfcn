...
!     Timing 
      PTS_TINI = 0.0                ! Initialization time
      PTS_TEV = 0.0                 ! Total evolution time
      PTS_EVN = 0                   ! Total calls
      PTS_ION = 0
      PTS_TIO = 0.0
...
!     stamp the log
      ...
      return
      end subroutine stat_pts_init
!-----------------------------------------------------------------------
!     Save Time-history
      subroutine PTS_COMPUTE
...
!       logfile
        if (NIO.eq.0) write(6,*) 'Saving Time history'
...
!       pressure
        call stat_pts_compute(tmpvel,slvel,tmppr)           ! Doesn't work for 2D           

!     simple timing
        ltim_init = dnekclock() - ltim_init

!     update timing and counters
        PTS_TEV = PTS_TEV + ltim_init
        PTS_EVN = PTS_EVN + 1

      endif

      return
      end subroutine PTS_COMPUTE
...
!        prabal. PTSTAT has variable name conflicts   
!        Update timing and counters   
!         PTS_TIO = PTS_TIO + ltim_init
!         PTS_ION = PTS_ION + 1            
