...
!     default values
      ...
      t_stat_comp   = 100           ! Time dependent stats compute time
      t_stat_avgiter= 10            ! Time dependent stats averaging iterations
...
!     broadcast data
      ...
      call bcast(t_stat_comp,       ISIZE)
      call bcast(t_stat_avgiter,    ISIZE)
...
!       this routine uses number of scratch common blocks
        if (STAT_IFSTPTS.and.if3d) then           ! Doesn't work for 2D yet
          call stat_pts_init
        else
          STAT_IFSTPTS=.FALSE.
          if (nio.eq.0) write(6,*) 'Time history disabled'  
        endif
        icalld = 1
        return  
      endif    
...
!         Get time averaging parameters
          ...
          call STAT_COMPUTE(alpha,beta,iftimedep)

        endif    
!       time history    
        if (mod(ISTEP,STATPTS_COMP).eq.0) call PTS_COMPUTE

        if (mod(ISTEP,STAT_OUTP).eq.0) then

          call STAT_GL_COLLATION(iftimedep)     ! Communication
          call STAT_MFO_OUTFLD2D(iftimedep)     ! Output
          STAT_ATIME = 0.
          STAT_TSTART = time
          STAT_EVRN = 0
          STAT_TACC = 0.0
...
!     Time dependent stats calculation
      STAT_TDEP=.false.
      if ((STAT_TDEP).and.(icalld.gt.0)) then           ! prabal
        ...
        if (mod(ISTEP,T_STAT_COMP).LT.T_STAT_AVGITER) then
          ...
          if (COMPS.eq.T_STAT_AVGITER) then
            call STAT_GL_COLLATION(iftimedep)
            call STAT_MFO_OUTFLD2D(iftimedep)
            COMPS=0
          endif
        else
          comps=0
...
!-------------------------------------------------- 
!     Timing for Time history
!     simple timing
      ltim_min  = glmin(PTS_TINI,1)
      ltim_max  = glmax(PTS_TINI,1)
!     stamp logs
      if (NIO.eq.0) then
         write(6,*) 'Time history timning:'
         write(6,*) 'initialisation min/max: ',ltim_min, ltim_max
      endif
      ltim_min  = glmin(PTS_TEV,1)
      ltim_max  = glmax(PTS_TEV,1)
!     stamp logs
      if (NIO.eq.0) then
         write(6,*) 'evolution min/max:      ',ltim_min, ltim_max,
     $        PTS_EVN
      ...
      ltim_min  = glmin(PTS_TIO,1)
      ltim_max  = glmax(PTS_TIO,1)
!     stamp logs
      if (NIO.eq.0) then
         write(6,*) 'I/O min/max:            ',ltim_min, ltim_max,
     $        PTS_ION
...
!     perform local 1D integration on 1 variable
      subroutine stat_compute_1Dav1(lvar,npos,alpha,beta,iftimedep)
...
!     argument list
      logical iftimedep         ! time dependent/independent stats
      ...
      integer xyz               ! If coordinate field or regular statistics field
                                ! 0 ==> regular fld; 1 ==> x coordinate
                                ! 2 ==> y coordinate        
...
!     time average
      ...
      if (iftimedep) then
        call add2sxy(STAT_RAVGT(1,1,1,npos),alpha,rtmp,beta,e)
      else  
...
c----------------------------------------------------------------------
!     compute statistics
      subroutine STAT_COMPUTE(alpha,beta,iftimedep)
...
!     stamp logs
      if (NIO.eq.0) then
        if (iftimedep) then
          write(6,*) 'Time-dep 2D statistics compute'
        else
          write(6,*) '2D statistics compute'
        endif
      endif    
...
!     Store averaged grid for time dependent STATS
      ...
      if (iftimedep) then
        ...
        lnvar=0
!        if (STAT_IDIR.eq.1) then 
!          call stat_reshufflev(tmpvel(1,1,1,1,1),ym1(1,1,1,1),NELV)
!          call stat_reshufflev(tmpvel(1,1,1,1,2),zm1(1,1,1,1),NELV)
!        elseif (STAT_IDIR.eq.2) then 
!          call stat_reshufflev(tmpvel(1,1,1,1,1),xm1(1,1,1,1),NELV)
!          call stat_reshufflev(tmpvel(1,1,1,1,2),zm1(1,1,1,1),NELV)
!        elseif (STAT_IDIR.eq.3) then 
!          call stat_reshufflev(tmpvel(1,1,1,1,1),xm1(1,1,1,1),NELV)
!          call stat_reshufflev(tmpvel(1,1,1,1,2),ym1(1,1,1,1),NELV)
!        endif
!        
!        xyz = 1        
...
!       prabal. Just saving. No averaging
        call user_stat_coords(iftimedep)               ! save coordinates        
...
c$$$      enddo
c$$$!     for testing; end
      call stat_compute_1Dav1(slp(1,1,1,1),npos,alpha,beta,iftimedep)
...
c============================================================  
     ...
     $              dudx,dvdx,dwdx,lnvar,npos,alpha,beta,iftimedep)
...
!------------------------------
      ...
      do ii=0,collsteps-1
        ...
        call izero(rsum_procpos,nprocs)
     ...
     $      stat_lnum,nid)
        ...
        call icopy(tmp_procpos,rsum_procpos,nprocs)                     ! tmp_procpos == position
     
        call ibcastn(rsum_procpos,nprocs,np-1)           ! broadcasts the max receives for each process
                                                         ! for: pidmin <= nid <= pidmax     
        ...
        if ((nid.ge.pidmin).and.(nid.le.pidmax)) then         ! if NID within cycle
          stat_recno=rsum_procpos(nid-pidmin+1)               ! get no of receives
        endif

        do e2=1,nprocs
          if (logi_procpos(e2).gt.0) then
            pos_cnt=pos_cnt+1
            stat_procpos(pos_cnt)=tmp_procpos(e2)       ! position
          endif
...
!---------------------------------------------------------------------- 
      ...
      subroutine STAT_GL_COLLATION(iftimedep)
      ...
      real recxy(stat_lm1,stat_lm1,lelt*2)
      ...
      maxpos=ivlmax(stat_procpos,stat_snd_cnt)     ! max position number
      ...
      do e=1,stat_maxrec
        ...
        if (e.le.stat_recno) then
          len=(stat_lnum)*isize
          msgtag = tagofst+0 
          msg_id1 = irecv(msgtag,recmap,len)       ! rec mapping
        endif
...
!         Data  
          ...
          msgtag = tagofst+1  
          msg_id2 = irecv(msgtag,recdata,len)

!         Arc Length
          msgtag = tagofst+2    
          len=wdsize*recsize*stat_nm2*stat_nm3
          msg_id3 = irecv(msgtag,arclen,len)

!         prabal. right now just saving            
!         Recieve coordinates if time dependent stats 
!          if (iftimedep) then  
!            len=wdsize*recsize*stat_nm2*stat_nm3*2
!            msgtag = tagofst+3
!            msg_id4 = irecv(msgtag,recxy,len)
!          endif  

        endif           ! e.le.stat_recno

c------ Send data
        do e2=1,cnt2
          dstid=snds(e2)
          call senddata(dstid,iftimedep)
        enddo

c------ Receive data and arclength and coordinates
        if (e.le.stat_recno) then
          call msgwait(msg_id2)               ! wait to receive data

!          prabal. right now just saving           
!          if (iftimedep) call msgwait(msg_id4)   ! wait to receive xy coordinates

          call colldata(recmap,recsize,recdata,recxy,iftimedep)
      ...
      if (if3d) then
        ...
        do e=1,stat_lnum
          if (stat_own(e).eq.nid) then
            do e2=1,stat_lvar
              if (iftimedep) then
                call invcol2(stat_ravgt(1,1,e,e2),
...
!           prabal. Right now just saving            
!            if (iftimedep) then
!              call invcol2(stat_xmt(1,1,e),
!     $              stat_abm1d(1,1,e),len)
!              call invcol2(stat_ymt(1,1,e),
!     $              stat_abm1d(1,1,e),len)
!            endif
          endif
        enddo
...
!---------------------------------------------------------------------- 
      ...
      subroutine senddata(dstid,iftimedep)
      ...
      real arclen(stat_lm1,stat_lm1,lelt)
      real sndxy(stat_lm1,stat_lm1,lelt*2)
      ...
      do e=1,stat_lvar
        do e2=1,lsels
          pos = (e-1)*lsels+e2
          if (iftimedep) then
            call copy(snddata(1,1,pos),
     $           stat_ravgt(1,1,indmap(e2),e),len)
            call rzero(stat_ravgt(1,1,indmap(e2),e),len)
          else
            call copy(snddata(1,1,pos),
     $           stat_ruavg(1,1,indmap(e2),e),len)
...
!     prabal. Right now just saving      
!      if (st_index.eq.2) then
!        do e2=1,lsels    
!          call copy(sndxy(1,1,e2),stat_xmt(1,1,indmap(e2)),len)
!        enddo    
!        do e2=lsels+1,2*lsels    
!          call copy(sndxy(1,1,e2),stat_ymt(1,1,indmap(e2)),len)
!        enddo
!      endif 
      ...
      msgtag = tagofst+2
      call csend(msgtag,arclen,len,dstid,0)          ! send arc length

!     prabal. right now just saving      
!      if (st_index.eq.2) then
!        len=stat_nm2*stat_nm3*lsels*2*wdsize
!        msgtag = tagofst+3
!        call csend(msgtag,sndxy,len,dstid,0)        ! send xy coordinates
!      endif
...
c--------------------------------------------------
      ...
      subroutine colldata(recmap,n,recdata,recxy,iftimedep)
      ...
      do e=1,nvars
        do e2=1,n
          pos = (e-1)*n + e2
          if (iftimedep) then
            call add2(stat_ravgt(1,1,recmap(e2),e),
     $             recdata(1,1,pos),len)
          else
            call add2(stat_ruavg(1,1,recmap(e2),e),
     $             recdata(1,1,pos),len)
          endif 
        enddo
      enddo
...
!        do e2=1,n
!          pos = e2
!          call add2(stat_xmt(1,1,recmap(e2)),
!     $             recdata(1,1,pos),len)
!          pos = n + e2  
!          call add2(stat_ymt(1,1,recmap(e2)),
!     $             recdata(1,1,pos),len)
!        enddo
!      endif
