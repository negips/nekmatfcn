...
!     default values
      ...
      SURF_FLDOUT   = 10000         ! Surface fld output interval
...
!----------------------------------------------------------------------  
      ...
      parameter (arr_size=lx1*ly1*lz1*lelt)
      ...
      integer rsum_procpos(arr_size)

      integer pidmin,pidmax
...
!------------------------------
      ...
      do ii=0,collsteps-1
        ...
        call izero(rsum_procpos,arr_size)
        ...
        pidmin=poffset                ! smallest pid for this cycle
        pidmax=poffset+nprocs-1       ! largest pid for this cycle
        ...
        call ivgl_running_sum(rsum_procpos,tmp_procpos,nprocs)          ! running sum across all processes
        call icopy(tmp_procpos,rsum_procpos,nprocs)
     
        call ibcastn(rsum_procpos,nprocs,np-1)           ! broadcasts the max receives for each process
                                                         ! for: poffset <= nid < pffset+nprocs       
        tmp_max=ivlmax(rsum_procpos,nprocs)
        if (tmp_max.gt.SURF_MAXREC) then
          SURF_MAXREC=tmp_max                    ! update max number of receives
        endif                                    ! across all processes.

!       if (nid) is within the processors being checked then
!       get total no of receives for (nid)
        if ((nid.ge.pidmin).and.(nid.le.pidmax)) then
          SURF_RECNO=rsum_procpos(nid+1-poffset)    ! get how many NID receives
        endif

        do e2=1,nprocs
          if (logi_procpos(e2).gt.0) then
            pos_cnt=pos_cnt+1
            SURF_PROCPOS(pos_cnt)=tmp_procpos(e2)
          endif
...
!---------------------------------------------------------------------- 
      ...
      integer msgtag
      integer tagofst                                  ! offset for msg tags
      parameter (tagofst=140)                          ! In case of leakage in communication
      integer ierr
      integer dstid
      integer maxpos
...
!     Collate Surface integral values
     ...
     $       3*SURF_LVAR*SURF_MAXTSAVES*MAXSURFS)
      ...
      maxpos=ivlmax(SURF_PROCPOS,SURF_SND_CNT)     ! local comm. steps for sending 
...
!       Waiting to receive data and arclength
        if (c_step.le.SURF_RECNO) then
          ...
          call COLLSURFDATA(recmap,rec_dist,recdata,arclen)          ! debugging
        endif
...
!     debugging
      ...
      ncalls = ncalls+1
      write(fname,'(A10,I5.5,A1,I2.2,A4)') 'surf_debug',nid,'_',
     $    ncalls,'.out'

!      uid = 3003+nid
!      open(unit=uid,FILE=fname,status='unknown',form='formatted')

!      write(uid,'(7(A7,2x))') '#Scount','Ofst','pos','it','svar',
!     $    'cntr','len' 
        ...
        do svar=1,SURF_LVAR
          do it=1,SURF_MAXTSAVES
            ...
            do e=1,scount
              ...
              cntr=cntr+1
!             debugging 
!              write(uid,'(7(I7,2x))') e,ofst,pos,it,svar,cntr,len 
...
!---------------------------------------------------------------------- 
      ...
      common /surfs_out/ wallxy,wallvar
      ...
      integer ioff                              ! offset for byte_set_view
      integer recpos                            ! rec position
      integer hdrofst                           ! offset for header
      integer tstmpofst                         ! timestamp offset
      integer intgofst                          ! integral vals offset
      integer xyofst                            ! xy offset
      integer fldofst                           ! current field offset
      integer ibsw_out                          ! if byte swap
      ...
      integer s_lels                            ! total local elements
      integer igl_running_sum                   ! running sum
      integer s_comm_pos                        ! send position
      integer iglmax                            ! function global vector max
      integer s_comm_max                        ! maximum no of processes with surface elements 
!---------------------------------------- 

      sout_timing = dnekclock()

      s_wdsizeio=WDSIZE                              ! double precision
      s_isizeio=ISIZE
      s_csizeio=CSIZE
...
! 1001 format (I5.5,2X,E14.6E3,2X,E14.6E3,2X,E14.6E3)    ! nid,x,y,p
     ...
     $              "(E14.6E3,2X))"
      ...
      call blank(shdr,132)
      recpos=1
      tstmpofst=SURF_NSAVES*S_WDSIZEIO
      hdrofst=132*S_CSIZEIO          ! hdr
      intgofst=NDIM*SURF_LVAR*SURF_NSAVES*S_WDSIZEIO

      totpts=0

      if (ifbinary) then 
        if (nid.eq.0) write(6,*) 'Output Surface history: Binary file'
      endif
...
!     File I/O
 ...
 9002   format (A9,I2.2,A1,E14.8E2,A4)
        ...
        if (nid.eq.0) then
!         STREAMIO is a fortran 2003 feature 
          open(unit=funits(i),file=fname,
     $            access='stream')

          write(6,*) 'SURF STATS: File open ',fname,funits(i)

!         Write hdr
          write(shdr,9003) '#',s_wdsizeio,i,LX1, 
     $    SURF_TUNQ(i),'XS',SURF_LVAR,SURF_MAXTSAVES,TIME

 9003     format (A1,2X,I2.2,2X,I3.3,2X,I2.2,2X,I5.5,2X,A2,I2.2,2X,
     $        I4.4,2X,E14.6E3)

          recpos=1
          write(funits(i),pos=recpos) shdr              

!         write Time stamps
          ioff=hdrofst
          recpos=ioff+1
          write(funits(i),pos=recpos) (SURF_TSTAMP(ix),ix=1,SURF_NSAVES)

          if (ifbinary) then
!           write integral values
            do sel=1,SURF_LVAR
              do scomp=1,NDIM
                fldofst=((sel-1)*NDIM*SURF_NSAVES
     $                  +(scomp-1)*SURF_NSAVES)*S_WDSIZEIO
                ioff=hdrofst+tstmpofst+fldofst
                recpos=ioff+1
                write(funits(i),pos=recpos)
     $            (SURF_INTV_HIST(ix,scomp,sel,i),ix=1,SURF_NSAVES)
              enddo  ! scomp   
            enddo    ! sel   
          endif      ! ifbinary
        endif        ! nid.eq.0   
      ...
      counter=0
      do isf = 1,NSURFS
        xyofst=2*SURF_TUNQ(isf)*LX1*S_WDSIZEIO
        cntr=0 
        do sel = 1,SURF_LNUM(isf)
...
!           XM1/YM1
            if (ifbinary) then
              fldofst=2*(cntr-1)*lx1*s_wdsizeio 
!             XM1 
              ioff=hdrofst+tstmpofst+intgofst+fldofst
              recpos=ioff+1 
              if (nid.eq.0) then    
                write(funits(isf),pos=recpos)
     $            (SURF_XM1(ix,sel,isf),ix=1,lx1)
              endif    

!             YM1 
              fldofst=fldofst+lx1*s_wdsizeio 
              ioff=hdrofst+tstmpofst+intgofst+fldofst
              recpos=ioff+1    
              if (nid.eq.0) then    
                write(funits(isf),pos=recpos)
     $            (SURF_YM1(ix,sel,isf),ix=1,lx1)
              endif    
            endif    ! ifbinary   
            counter=counter+LX1
          endif      ! SURF_OWN   
          ...
          do it=1,SURF_NSAVES
            ...
            do sel=1,SURF_LNUM(isf)
              if (SURF_OWN(sel,isf).eq.nid) then
                cntr=cntr+1   
                if (ifbinary) then
                  fldofst=((SURF_TUNQ(isf)*(svar-1)*LX1*SURF_NSAVES  ! previous variables
     $             + SURF_TUNQ(isf)*LX1*(it-1)                       ! previous time slices 
     $             + (cntr-1)*LX1 ))*S_WDSIZEIO                      ! previous elements

                  ioff=hdrofst+tstmpofst+intgofst+xyofst+fldofst
                  recpos=ioff+1
                  if (nid.eq.0) then
                    write(funits(isf),pos=recpos) 
     $                (SURF_AVG_HIST(ix,sel,it,svar,isf),ix=1,lx1)
                  endif 
                endif   ! ifbinary
              endif     ! SURF_OWN  
            enddo       ! sel
      ...
      s_lels = ivlsum(SURF_MEMCOUNT,NSURFS)     ! no of local surface elements (all surfaces)
      if (s_lels.ge.1) s_lels = 1               ! pid has elements
      if (nid.eq.0) s_lels = 0                  ! already written to file for nid.eq.0
      s_comm_pos = igl_running_sum(s_lels)      ! running sum over processors
      s_comm_max = iglmax(s_comm_pos,1)
...
!         Don't need handshaking now
  
!         count number
          length=NSURFS*isize
          msgtag=tagofst+ip  
          call crecv(msgtag,cnt_rem,length)
!         get corrdinates 
          counter=ivlsum(cnt_rem,NSURFS)
          totpts=totpts+counter*LX1
          if (counter.gt.0) then                                 ! This if is now redundant
            ...
            do isf = 1,NSURFS
              if (ifbinary) then
                do sel = 1,cnt_rem(isf)
                  selno=selno+1
                  fldofst=(2*cnt_rsum(isf)*LX1                   ! Previous processors
     $                    +2*(sel-1)*LX1)*S_WDSIZEIO             ! Previous elements
!                 XM1 
                  ioff=hdrofst+tstmpofst+intgofst+fldofst
                  recpos=ioff+1
                  write(funits(isf),pos=recpos)
     $              (wallxy(ix,sel,1),ix=1,lx1) 
                ...
                do svar=1,SURF_LVAR 
                  do it=1,SURF_MAXTSAVES
                    do sel=1,cnt_rem(isf)
                      ...
                      fldofst=(SURF_TUNQ(isf)*(svar-1)*LX1*SURF_NSAVES ! Previous variables
     $                  + SURF_TUNQ(isf)*LX1*(it-1)                    ! Previous slices 
     $                  + cnt_rsum(isf)*LX1                            ! Previous processors 
     $                  + (sel-1)*LX1)*S_WDSIZEIO                      ! Previous elements 

                      ioff=hdrofst+tstmpofst+intgofst+xyofst+fldofst
                      recpos=ioff+1  
                      write(funits(isf),pos=recpos) 
     $                  (wallvar(ix,selno2),ix=1,lx1)
...
!         If pid has surface elements
!         And communication position matches    
          if ((ip.eq.s_comm_pos).and.(s_lels.gt.0)) then   

            length = NSURFS*isize
            msgtag=tagofst+ip
            call csend(msgtag,SURF_UNQ,length,0,0)
...
!             send data
          ...
          endif         ! ip.eq.s_comm_pos

          call nekgsync   
        enddo           ! ip=1,s_comm_max      
...
!     $                       YM1(tix,tiy,tiz,tie),dmin,dmin-dp
            ...
            if (nid.eq.0) then
               write(6,*) 'Process with Cp reference point:,',rip
            endif
      endif
...
!----------------------------------------------------------------------
      ...
      subroutine SURF_OUTPUT_MAPPING
...
!      include 'mpif.h'
      ...
      write(str,'(i5.5)') np     
      if  (nid.eq.0) then
      open(unit=1001,file='surf_map_'//str//'.out')
      
      tmp_rec_len(1)=SURF_SND_CNT
      if (tmp_rec_len(1).eq.0) tmp_rec_len(1)=1
      write(1001,*) 'NID/SND_CNT/RECNO/MAXREC:',nid,SURF_SND_CNT,
     $    SURF_RECNO,SURF_MAXREC
      write(1001,*) 'PROCID:',(SURF_PROCID(jj), jj=1,tmp_rec_len(1))
      write(1001,*) 'PROCPOS:',(SURF_PROCPOS(jj), jj=1,tmp_rec_len(1))
     ...
     $         tmp_rec_len(2), tmp_rec_len(3)
      ...
      close(1001)
          ...
          len=3*isize
          tmp_rec_len(1) = SURF_SND_CNT
          tmp_rec_len(2) = SURF_RECNO
          tmp_rec_len(3) = SURF_MAXREC
          call csend(msg_id,tmp_rec_len,len,p_recid,0)     ! send data length

          msg_id  = 1
          len=lelt*isize
          call csend(msg_id,SURF_PROCID,len,p_recid,0)      ! send procid

          msg_id = 2
          call csend(msg_id,SURF_PROCPOS,len,p_recid,0)     ! send procpos    

      endif

      end subroutine SURF_OUTPUT_MAPPING
